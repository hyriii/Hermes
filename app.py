...
def fix_text(text: str) -> str:
    """Centralized Arabic text fixing with arabic_reshaper only"""
...
def reshape_arabic(text: str) -> str:
    """Reshape Arabic text for proper display - REMOVED get_display() to prevent double-reversing"""
    if not ARABIC_SUPPORT or not arabic_reshaper:
        return text
    try:
        if any('\u0600' <= c <= '\u06FF' for c in text):
            # Golden Rule: arabic_reshaper.reshape(text) ONLY - NO get_display()
...
def create_creative_pptx(book_title, sections_data, output_mode):
    ...
    """
    st.write("🎨 Creating comprehensive creative PPTX presentation...")
    ...
    toc_items = []
    for i, section in enumerate(sections_data[:12]):  # Show up to 12 sections
    ...
    for section_idx, section in enumerate(sections_data):
        ...
        for i, (shape_spec, content_part) in enumerate(zip(layout['shapes'], content_parts[:len(layout['shapes'])])):
            ...
            for line in lines[:8]:  # Limit lines per content box
                if line.strip():
...
with tab2:
    ...
    if uploaded_file:
        ...
        with st.spinner("Extracting text..."):
            ...
            if pdf_text:
                st.toast(f"✓ Extracted {len(pdf_text)} chars", icon="✅")
                st.session_state['pdf_text'] = pdf_text
                st.session_state['pdf_page_range'] = page_range  # Store page range for API context
                ...
                if page_range:
                    st.error("Could not extract text. Check your page range settings.")
    ...
    if combined_text and len(combined_text.strip()) >= 50:
        ...
        with col2:
            ...
            if st.button("⚖️ ✨ Start Summary", type="primary", use_container_width=True):
                with st.spinner("🔮 Hermes is analyzing the content..."):
                    try:
                        # CRITICAL: Check if combined_text is NOT empty before sending to AI
                        if not combined_text or len(combined_text.strip()) < 100:
                            st.error("❌ Failed to extract text from PDF - no content available for analysis")
                            st.stop()

                        # DEBUG: Show exactly what text was extracted
                        st.text_area("Debug: Extracted Text", combined_text, height=200)

                        # Use the combined text for analysis
                        pages = split_text_by_pages(combined_text)
                        st.toast(f"📑 Processing {len(pages)} pages...", icon="📑")
                        
                        sections_data = []
                        for i, page in enumerate(pages):
                            # Quality check: Show chunk processing with language detection
                            chunk_language = detect_dominant_language(page['text'])
                            language_name = "Arabic" if chunk_language == 'ar' else "English"
                            st.write(f'Processing chunk {i+1} - Language: {language_name}')
                        ...
                        if sections_data:
                            ...
                            for sd in sections_data:
                                full_summary += sd['content'] + "\n\n"
                            
                            st.session_state['summary_output'] = full_summary
                            st.toast("✅ Analysis complete! The wisdom of Hermes is revealed!", icon="⚖️")

                            st.markdown("### 📋 Summary Result")
                            # UI Rendering: Use st.markdown with RTL CSS for proper right-to-left alignment
                            processed_text = fix_text(full_summary)
                            if is_arabic_text(full_summary):
                                st.markdown(f'<div style="text-align: right; direction: rtl;">{processed_text}</div>', unsafe_allow_html=True)
                            else:
                                st.markdown(processed_text)
                            
                            # Print statement for page processing confirmation
                            st.write(f"🔄 Processing pages {page_from} to {page_to}..." if page_from and page_to else "🔄 Processing all pages...")

                            # DEBUG: Data verification before PPTX creation
                            st.write(f"DEBUG: Sections found: {len(sections_data)}")
                            for i, section in enumerate(sections_data):
                                content_preview = section.get('content', '')[:100] + "..." if len(section.get('content', '')) > 100 else section.get('content', '')
                                st.write(f"Section {i+1}: {len(section.get('content', ''))} chars - Preview: {content_preview}")

                            # DEEP SEARCH: Comprehensive section aggregation with multiple fallback strategies
                            all_sections_data = []
                            section_counter = 1

                            st.write("🔍 DEEP DEBUG: Processing sections_data with multiple strategies...")

                            # Strategy 1: Check if we have any data at all
                            if not sections_data:
                                st.error("❌ CRITICAL: sections_data is empty! AI analysis failed.")
                                st.stop()

                            for idx, chunk_result in enumerate(sections_data):
                                st.write(f"📦 Chunk {idx+1} Analysis:")
                                st.write(f"   Type: {type(chunk_result)}")

                                if not isinstance(chunk_result, dict):
                                    st.write(f"   ❌ Not a dict, skipping")
                                    continue

                                if 'content' not in chunk_result:
                                    st.write(f"   ❌ No 'content' key, skipping")
                                    continue
                                ...
                                if len(content.strip()) < 50:
                                    st.write(f"   ❌ Content too short, skipping")
                                    continue

                                st.write(f"   Content preview: {repr(content[:150])}")

                                # Strategy 2: Multiple parsing approaches
                                sections_found = []

                                # Approach A: Standard [SECTION] parsing
                                if '[SECTION' in content.upper():
                                    st.write("   🎯 Found [SECTION] markers - using standard parsing")
                                    sections_split = re.split(r'\[SECTION\s*([^\]]+)\]', content)
                                    st.write(f"   Split into {len(sections_split)} parts")
                                    ...
                                    for i in range(1, len(sections_split), 2):
                                        if i+1 < len(sections_split):
                                            ...
                                            if title and content_part:
                                                ...
                                                st.write(f"   ✅ Extracted: '{title[:50]}...' -> {len(content_part)} chars")

                                # Approach B: Fallback - split by common section markers
                                if not sections_found:
                                    st.write("   🔄 No [SECTION] markers found - trying fallback parsing")
                                    fallback_patterns = [
                                        r'\n##\s*([^\n]+)\n',  # ## Title
                                        r'\n###\s*([^\n]+)\n', # ### Title
                                        r'Section\s*\d+:\s*([^\n]+)', # Section X: Title
                                        r'\n\s*([A-Z][^.\n]{10,50})\n' # Capitalized sentences as titles
                                    ]

                                    for pattern in fallback_patterns:
                                        matches = re.findall(pattern, content, re.MULTILINE | re.IGNORECASE)
                                        if matches:
                                            st.write(f"   🎯 Found {len(matches)} sections with pattern: {pattern}")
                                            ...
                                            for i in range(1, len(parts), 2):
                                                if i+1 < len(parts):
                                                    ...
                                                    if title and len(content_part) > 20:
                                                        sections_found.append((title, content_part))
                                                        st.write(f"   ✅ Fallback extracted: '{title[:30]}...' -> {len(content_part)} chars")
                                            break

                                # Approach C: Emergency fallback - split by paragraphs
                                if not sections_found and len(content.strip()) > 200:
                                    st.write("   🚨 Emergency fallback - splitting by paragraphs")
                                    paragraphs = [p.strip() for p in content.split('\n\n') if p.strip() and len(p.strip()) > 30]
                                    for i, para in enumerate(paragraphs[:5]):  # Take first 5 substantial paragraphs
                                        ...
                                        if len(sentences) > 1:
                                            ...
                                            if len(content_part) > 20:
                                                ...
                                                st.write(f"   ✅ Emergency extracted: '{title[:30]}...' -> {len(content_part)} chars")

                                # Strategy 3: Create slides from found sections
                                for title, content_part in sections_found:
                                    if title and content_part and len(content_part.strip()) > 10:
                                        ...
                                        section_counter += 1
                                        st.write(f"   🎉 Added section {section_counter-1}: '{title[:40]}...'")

                            # Strategy 4: Final validation
                            st.write(f"📊 FINAL RESULT: {len(all_sections_data)} sections aggregated")

                            if not all_sections_data:
                                st.error("❌ CRITICAL: No sections could be extracted from AI responses!")
                                st.error("This means the AI is not generating content in any recognizable format.")
                                st.error("Possible causes:")
                                st.error("1. AI API key invalid or quota exceeded")
                                st.error("2. AI model not responding properly")
                                st.error("3. Network connectivity issues")
                                st.error("4. AI prompt not clear enough")
                                st.stop()

                            # Show summary of extracted sections
                            st.write("📋 Extracted Sections Summary:")
                            for i, section in enumerate(all_sections_data[:3]):  # Show first 3
                                st.write(f"   {i+1}. '{section['title'][:50]}...' ({len(section['content'])} chars)")

                            if len(all_sections_data) > 3:
                                st.write(f"   ... and {len(all_sections_data) - 3} more sections")

                            # Generate comprehensive creative PPTX covering entire PDF content
                            ...
                            # ========== SELF-TESTING (Agent Mode) ==========
                            st.markdown("### 🔍 Self-Testing Results")
                            inspection_results = inspect_pptx_arabic_correctness(ppt_file)

                            # Display inspection results
                            if inspection_results['status'] == 'PASS':
                                st.success("✅ Arabic text inspection PASSED - PPTX is correctly formatted!")
                                st.write(f"📊 Slides: {inspection_results['total_slides']}")
                                if inspection_results['arabic_text_found']:
                                    st.write("🔤 Arabic text detected and properly formatted")
                            else:
                                st.error("❌ Arabic text inspection FAILED - Issues detected!")
                                st.write(f"📊 Slides: {inspection_results['total_slides']}")
                                if inspection_results['arabic_text_found']:
                                    st.write("🔤 Arabic text found but may have formatting issues")

                                # Show recommendations
                                if inspection_results['recommendations']:
                                    st.warning("🔧 Recommendations:")
                                    for rec in inspection_results['recommendations']:
                                        st.write(f"• {rec}")

                                # Attempt to regenerate with fixes using the creative generator
                                st.info("🔄 Attempting to regenerate PPTX with fixes...")
                                ppt_file_fixed = create_creative_pptx(
                                    book_title=book.get('title', 'Unknown'),
                                    sections_data=all_sections_data,
                                    output_mode=output_mode
                                )
                                ...
                                # Re-inspect
                                inspection_results_fixed = inspect_pptx_arabic_correctness(ppt_file_fixed)

                                if inspection_results_fixed['status'] == 'PASS':
                                    st.success("✅ Fixed PPTX generated successfully!")
                                    ppt_file = ppt_file_fixed  # Use the fixed version
                                else:
                                    st.error("❌ Unable to fix all issues automatically. Manual review recommended.")
        ...
        st.warning("⚠️ Text too short (minimum 50 characters)")
    else:
